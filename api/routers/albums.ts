...
albumRouter.delete("/:id", authentication, async (req, res, next) => {
    try {
        ...
        if (tracks.length > 0) {
            res.status(400).send({error: "Cannot delete an album that has related tracks"});
            return;
        }

        const user = (req as RequestWithUser).user;
        const deleteAlbum = await Album.deleteOne({_id: id, isPublished: false, user: user._id});

        if (deleteAlbum.deletedCount === 0) {
            res.status(400).send({error: "Unable to delete an album that has already been published or you didn't create it"});
            return;
        }
