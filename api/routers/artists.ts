...
artistRouter.delete("/:id", authentication, async (req, res, next) => {
    try {
        ...
        if (albums.length > 0) {
            res.status(400).send({error: "Cannot delete an artist who has albums associated with him or her"});
            return;
        }

        const user = (req as RequestWithUser).user;
        const deleteArtist = await Artist.deleteOne({_id: id, isPublished: false, user: user._id});

        if (deleteArtist.deletedCount === 0) {
            res.status(400).send({error: "Unable to delete an artist that has already been published or you didn't create it"});
            return;
        }
