...
import {
    ...
    selectAdminArtistDeleteLoading, selectAdminArtistFetchLoading,
    selectAdminArtists
...
const ArtistAdminList = () => {
    ...
    const columns: GridColDef<IArtistAdmin>[] = [
        ...
        { field: "isPublished", headerName: "Publication status", width: 150 },
        {
            ...
            renderCell: (fields) => (
                <>
                    <IconButton onClick={() => deleteArtist(fields.row._id)} disabled={changeLoading}>
                        <ClearIcon/>
                    </IconButton>
                    <IconButton onClick={() => changeStatusArtist(fields.row._id)} disabled={deleteLoading}>
                        <EditIcon/>
                    </IconButton>
                </>
            )
